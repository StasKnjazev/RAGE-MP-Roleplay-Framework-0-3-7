...
class LSCCostumStore : Script
{
    ...
    {
        NAPI.Marker.CreateMarker(27, new Vector3(-355.9738, -130.3064, 39.43887 - 1.0), new Vector3(), new Vector3(), 0.8f, new Color(255, 255, 255, 155));
    }

    public static void PressKeyY(Client player)
    {
        if(Main.IsInRangeOfPoint(player.Position, new Vector3(-355.9738, -130.3064, 39.43887), 1.0f))
        {
            ...
            InteractMenu.CreateMenu(player, "LKW_STORE_ITENS", "LSPC Store", "~b~Erhältliche Artikel:", true, JsonConvert.SerializeObject(menu_item_list), false);
        }
    }
