...
class MedicSnackStore : Script
{
    ...
    {
        NAPI.Marker.CreateMarker(27, new Vector3(350.656, -584.5171, 28.79147 - 1.0), new Vector3(), new Vector3(), 0.8f, new Color(255, 255, 255, 155));
        NAPI.Marker.CreateMarker(27, new Vector3(1834.0409, 3673.068, 34.271034 - 1.0), new Vector3(), new Vector3(), 0.8f, new Color(255, 255, 255, 155));
    }

    public static void PressKeyY(Client player)
    {
        if(Main.IsInRangeOfPoint(player.Position, new Vector3(350.656, -584.5171, 28.79147), 1.0f))
        {
            ...
            InteractMenu.CreateMenu(player, "MEDIC_SNACK_STORE_ITENS", "Medic Snack Store", "~b~Erhältliche Artikel:", true, JsonConvert.SerializeObject(menu_item_list), false);
        }
        else if (Main.IsInRangeOfPoint(player.Position, new Vector3(1834.0409, 3673.068, 34.271034), 1.0f))
        {
            List<dynamic> menu_item_list = new List<dynamic>();
            menu_item_list.Add(new { Type = 1, Name = "~c~1x~w~ Hamburger", Description = "American Style Hamburger.", RightLabel = "~g~$" + PRICE_HAMBURGER.ToString("N0") + "" });
            menu_item_list.Add(new { Type = 1, Name = "~c~1x~w~ Kaffee", Description = "Ein frischen Kaffee zum mitnehmen.", RightLabel = "~g~$" + PRICE_KAFFEE.ToString("N0") + "" });
    ...
    {
        if(callbackId == "MEDIC_SNACK_STORE_ITENS")
        {
            if (selectedIndex == 0)
            {
