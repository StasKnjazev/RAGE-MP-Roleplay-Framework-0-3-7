...
namespace RAGEMP_TsVoice
{
    public class Teamspeak : Script
    {
        ...
        public void ChangeVoiceRange(Client client)
        {
            ...
            NAPI.ClientEvent.TriggerClientEvent(client, "UpdateVoiceImage");
        }

        [RemoteEvent("GTA5Voice_CallPhoneAccepted")]
        public void CallPhoneAccepted(Client client, Client caller)
        {
            caller.GetSharedData(client.GetSharedData("TsName"));

            client.TriggerEvent("CALL_IS_STARTED", caller.GetSharedData("TsName"));
            caller.TriggerEvent("CALL_IS_STARTED", client.GetSharedData("TsName"));
        }

        [RemoteEvent("GTA5Voice_CallPhoneEnded")]
        public void CallPhoneEnded(Client client, Client caller)
        {
            client.ResetData("CALLING_PLAYER_NAME");
            caller.ResetData("CALLING_PLAYER_NAME");
            return;
        }
