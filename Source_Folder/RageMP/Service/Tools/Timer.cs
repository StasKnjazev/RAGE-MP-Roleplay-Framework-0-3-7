...
public class TimerEx : Script
{
    ...
    [Command("ttimer1")]
    public void timerTestings(Client player)
    {
        int index = 10;
        // Without existing method //
        TimerEx.SetTimer(() => { NAPI.Notification.SendNotificationToPlayer(player, "[TIMER3] Bonus is da best"); }, 1000, 10);
    }
    ...
    public void timerTesting(Client player)
    {
        ...
        teste = TimerEx.SetTimer(() => {
            index++;

            NAPI.Notification.SendNotificationToPlayer(player,"~c~index >> ~g~" + index + "");

            if (index == 10)
            {

                teste.Kill();
                NAPI.Notification.SendNotificationToPlayer(player,"~c~index >> "+index+" ~y~ STOPED;");
            }
...
/* Examples: 
	...
	private void testTimerFunc ( Client player, string text ) {
		NAPI.Notification.SendNotificationToPlayer ( player, "[TIMER] "+text );
	}
	...
	public void timerTesting ( Client player ) {
		...
		// Without existing method //
		Timer.SetTimer ( () => { NAPI.Notification.SendNotificationToPlayer ( player, "[TIMER3] Bonus is da best" ); }, 1000, 0 );
	}
*/
