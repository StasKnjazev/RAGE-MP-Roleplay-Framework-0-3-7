...
public class LostMCCustom : Script
{
    ...
    public LostMCCustom()
    {
        ...
        ls_custom.Add(new LostMCCustomEnum { position = new Vector3(735.5488, -1071.827, 22.23297), in_use = false, vehicle = null });
        ls_custom.Add(new LostMCCustomEnum { position = new Vector3(731.3914, -1088.885, 22.16902), in_use = false, vehicle = null });
        //ls_custom.Add(new LostMCCustomEnum { position = new Vector3(-334.7063, -136.8645, 39.00962), in_use = false, vehicle = null });

        foreach (var ls in ls_custom)
        {
            ls.label_position = API.Shared.CreateTextLabel("Lost MC Tuning", ls.position + new Vector3(0, 0, 1.2f), 10.0f, 8.2f, 4, new Color(255, 255, 255, 255), false, 0);
    ...
    public class LostMCCustomEnum : IEquatable<LostMCCustomEnum>

    {
        ...
        public override bool Equals(object obj)
        {
            ...
            LostMCCustomEnum objAsPart = obj as LostMCCustomEnum;
            if (objAsPart == null) return false;
            else return Equals(objAsPart);
        }

        public bool Equals(LostMCCustomEnum other)
        {
    ...
    public static List<LostMCCustomEnum> ls_custom = new List<LostMCCustomEnum>();
    ...
    public static void PressKeyE(Client player)
    {
        ...
        foreach (var ls in ls_custom)
        {
            if (FactionManage.GetPlayerGroupType(player) == 7)
            {
                if (Main.IsInRangeOfPoint(player.Position, ls.position, 3.0f) && ls.in_use == false && player.IsInVehicle && player.VehicleSeat == -1)
            {
                ...
                vehicle.SetData("lscustom_color_2", API.Shared.GetVehicleSecondaryColor(player.Vehicle));
    ...
    public static void Display_LostMCCustom(Client player)
    {
        if (player.IsInVehicle && player.VehicleSeat == -1)
        {
            ...
            foreach (var ls in ls_custom)
            {
                if (ls.vehicle == vehicle)
                {
                    ...
                    vehicle.SetData("lscustom_color_1", vehicle.PrimaryColor);
                    vehicle.SetData("lscustom_color_2", API.Shared.GetVehicleSecondaryColor(vehicle));
                    vehicle.SetData("lscustom_neon", API.Shared.GetVehicleNeonState(vehicle));
                    vehicle.SetData("lscustom_neon_color", vehicle.NeonColor);
                    player.SetData("vehicle_mod_type", 0);
                    player.SetData("vehicle_mod_index", 0);
                    vehicle.SetData("primary_color_temp", vehicle.PrimaryColor);
                    vehicle.SetData("secundary_color_temp", API.Shared.GetVehicleSecondaryColor(vehicle));
                    Create_LostMCCustomMainMenu(player);
    ...
    public static void Create_LostMCCustomMainMenu(Client player)
    {
    ...
    public static void Hide_LostMCCustom(Client player)
    {
        if (player.IsInVehicle && player.VehicleSeat == -1)
        {
            ...
            foreach (var ls in LostMCCustom.ls_custom)
            {
