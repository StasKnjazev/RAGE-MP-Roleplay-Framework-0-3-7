    ...
    class Dashboard : Script
    {
                ...
                switch (type)
                {
                    case 0:
                        {// self inventory
                            ...
                            {//remove one item from player inventory
                                ...
                                {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Die Möglichkeit, Kanister auszulegen, ist vorübergehend deaktiviert", 5000);
                                    return;
                                }
                                ...
                                else if (nInventory.ClothesItems.Contains(item.Type))
                                {
                                    if (item.IsActive)
                                    {
                                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Sie müssen zuerst diese Kleidung ausziehen", 5000);
                                        return;
                                    }
                                ...
                                else if (nInventory.WeaponsItems.Contains(item.Type) || nInventory.MeleeWeaponsItems.Contains(item.Type) || item.Type == ItemType.StunGun)
                                {
                                    if (item.IsActive)
                                    {
                                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Sie müssen die Waffe aus Ihren Händen nehmen", 5000);
                                        return;
                                    }
                                    items.RemoveAt(index);
                                    Items.onDrop(player, new nItem(item.Type, 1, item.Data), null);
                                ...
                                if(player.IsInVehicle) {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Werfen Sie keine Gegenstände im Auto weg", 5000);
                                    return;
                                }
                                if (item.Count > 1)
                                {
                                    ...
                                    Trigger.ClientEvent(player, "openInput", "Gegenstand wegwerfen", "Menge eingeben", 3, "item_drop");
                                    return;
                                }
                            ...
                            else if (data == "use")
                            {
                                try
                                {
                                    ...
                                    if (player.HasData("CHANGE_WITH")) {
                                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Um Dinge zu benutzen, müssen Sie den Austausch von Dingen schließen", 5000);
                                        return;
                                    }
                                    ...
                                    case 2:
                                        {
                                            ...
                                            {
                                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Nicht genug Platz im Auto", 5000);
                                                return;
                                            }
                                            ...
                                            if (item.Count > 1)
                                            {
                                                ...
                                                player.SetData("ITEMTYPE", item.Type);
                                                player.SetData("ITEMINDEX", index);
                                                Trigger.ClientEvent(player, "openInput", "Gegenstand verschieben", "Menge eingeben", 3, "item_transfer_toveh");
                                                return;
                                            }
                                            if (item.Type == ItemType.Material)
                                            {
                                                ...
                                                {
                                                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Diese Anzahl von Material kann nicht geladen werden", 5000);
                                                    return;
                                                }
                                            }
                                    ...
                                    case 3:
                                        {
                                            ...
                                            {
                                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Dieser Artikel ist nicht für diesen Schrank vorgesehen.", 5000);
                                                return;
                                            }
                                            ...
                                            if (item.Count > 1)
                                            {
                                                ...
                                                player.SetData("ITEMTYPE", item.Type);
                                                player.SetData("ITEMINDEX", index);
                                                Trigger.ClientEvent(player, "openInput", "Gegenstand verschieben", "Menge eingeben", 3, "item_transfer_tosafe");
                                                return;
                                            }
                                            ...
                                            if (tryAdd == -1 || tryAdd > 0)
                                            {
                                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Nicht im Inventar", 5000);
                                                return;
                                            }
                                    ...
                                    case 4:
                                        {
                                            ...
                                            {
                                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Ein Kleiderschrank kann nur Kleidung aufbewahren", 5000);
                                                return;
                                            }
                                            if (Main.Players[player].InsideHouseID == -1) return;
                                            int houseID = Main.Players[player].InsideHouseID;
                                    ...
                                    case 5:
                                        {
                                            ...
                                            if (tryAdd == -1 || tryAdd > 0)
                                            {
                                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Der Bürger hat nicht genug Platz in sein Inventar", 5000);
                                                return;
                                            }
                                            ...
                                            if (item.Type == ItemType.BagWithDrill)
                                            {
                                                ...
                                                {
                                                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Der Bürger hat bereits was in der Hand.", 5000);
                                                    return;
                                                }
                                            ...
                                            else if (item.Type == ItemType.BagWithMoney)
                                            {
                                                if (target.HasData("HEIST_DRILL") || target.HasData("HAND_MONEY"))
                                                {
                                                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Du hast schon eine Tasche", 5000);
                                                    return;
                                                }
                                            ...
                                            if (item.Count > 1)
                                            {
                                                ...
                                                player.SetData("ITEMTYPE", item.Type);
                                                player.SetData("ITEMINDEX", index);
                                                Trigger.ClientEvent(player, "openInput", "Artikel übertragen", "Menge eingeben", 3, "item_transfer_toplayer");
                                                return;
                                            }
                                    ...
                                    case 6:
                                        {
                                            ...
                                            {
                                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Nicht genügend Platz", 5000);
                                                return;
                                            }
                                            ...
                                            if (item.Count > 1)
                                            {
                                                ...
                                                player.SetData("ITEMTYPE", item.Type);
                                                player.SetData("ITEMINDEX", index);
                                                Trigger.ClientEvent(player, "openInput", "Artikel übertragen", "Menge eingeben", 3, "item_transfer_tofracstock");
                                                return;
                                            }
                                    ...
                                    case 7:
                                        {
                                            ...
                                            if (keys.Count >= 5)
                                            {
                                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Du kannst nur Maximal 5 davon nehmen", 5000);
                                                return;
                                            }
                                            
                                            if (item.Type != ItemType.CarKey)
                                            {
                                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Gilt nur für Schlüssel", 5000);
                                                return;
                                            }
                                            ...
                                            OpenOut(player, keyringItems, "Schlüsselbund", 7);
                                            return;
                                        }
                                    case 8: // Оружейный сейф
                                        {
                                            ...
                                            {
                                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Ein Waffentresor kann nur Waffen aufbewahren", 5000);
                                                return;
                                            }
                                            if (Main.Players[player].InsideHouseID == -1) return;
                                            int houseID = Main.Players[player].InsideHouseID;
                    ...
                    case 2:
                        { // in car items
                            ...
                            if (item.Type == ItemType.BagWithDrill)
                            {
                                ...
                                {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Du hast bereits was in der Hand.", 5000);
                                    return;
                                }
                            ...
                            if (item.Count > 1)
                            {
                                ...
                                player.SetData("ITEMTYPE", item.Type);
                                player.SetData("ITEMINDEX", index);
                                Trigger.ClientEvent(player, "openInput", "Gegenstand nehmen", "Menge eingeben", 3, "item_transfer_fromveh");
                                return;
                            }
                    ...
                    case 3: // Взять
                        {
                            ...
                            if (item.Count > 1)
                            {
                                ...
                                player.SetData("ITEMTYPE", item.Type);
                                player.SetData("ITEMINDEX", index);
                                Trigger.ClientEvent(player, "openInput", "Gegenstand nehmen", "Menge eingeben", 3, "item_transfer_fromsafe");
                                return;
                            }
                    ...
                    case 6:
                        {
                            ...
                            if (item.Count > 1)
                            {
                                ...
                                player.SetData("ITEMTYPE", item.Type);
                                player.SetData("ITEMINDEX", index);
                                Trigger.ClientEvent(player, "openInput", "Gegenstand nehmen", "Menge eingeben", 3, "item_transfer_fromfracstock");
                                return;
                            }
                    ...
                    case 7:
                        { // keyring items
                            ...
                            if (tryAdd == -1 || tryAdd > 0)
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Sie haben nicht genug Platz", 5000);
                                return;
                            }
                    ...
                    case 20: 
                        {
                            if(Main.Players[player].AdminLVL >= 6 && Main.Players[player].InsideHouseID == -1) {
                                ...
                                if (nInventory.ClothesItems.Contains(item.Type) && item.IsActive == true)
                                {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Der Bürger muss diese Kleidung ausziehen.", 5000);
                                    return;
                                }
                                else if ((nInventory.WeaponsItems.Contains(item.Type) || nInventory.MeleeWeaponsItems.Contains(item.Type)) && item.IsActive == true)
                                {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Der Bürger muss diese Waffe aus seinen Händen nehmen.", 5000);
                                    return;
                                }
                                ...
                                if (item.Count > 1)
                                {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Das kannst du nicht aufgreiffen werden.", 5000);
                                    return;
                                }
                                nInventory.Add(player, item);
        ...
        public static void sendStats(Client player)
        {
            try
            {
                ...
                string number = (acc.Sim == -1) ? "Keine SIM-Karte" : Main.Players[player].Sim.ToString();
