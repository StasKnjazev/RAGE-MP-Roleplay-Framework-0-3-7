...
class Bus : Script
{
    ...
    private static List<String> BusWaysNames = new List<String>
        {
            ...
            "Western Ghetto - Airport - Central Square - Lawn Mower - Power Station - Taxopark",
            "Western Ghetto - Truck Driver - Eastern Ghetto - Collector - Mechanic",
    ...
    public void onResourceStartHandler()
    {
        try
        {
            ...
            Blip temp_blip = null;
            temp_blip = NAPI.Blip.CreateBlip(new Vector3(462.6476, -605.5295, 27.49518), 0);
            temp_blip.Sprite = 513;
            temp_blip.Name = "LS Bus Center";
            temp_blip.Color = 1;
            temp_blip.ShortRange = true;

            foreach (var station in BusStations)
                NAPI.TextLabel.CreateTextLabel($"~b~Bushaltestelle\n\n~y~{station.Key}", station.Value, 30f, 0.4f, 0, new Color(255, 255, 255), true, NAPI.GlobalDimension);
    ...
    {
        /*| bus1 | Saved Coordenates: 462.5794, -605.1766, 28.49113 Saved Rotation: 0.1124261, -0.245286, 33.12299
| bus | Saved Coordenates:  Saved Rotation:
| bu | Saved Coordenates: Saved Rotation:
| bus | Saved Coordenates:  Saved Rotation
| bus | Saved Coordenates:  Saved Rotation: 
| bus | Saved Coordenates:  Saved Rotation:
| bus | Saved Coordenates:  Saved Rotation: 
| bus | Saved Coordenates: Saved Rotation:
*/
        Main_VehicleManage.CreateVehicle(8, VehicleHash.Bus, new Vector3(462.5794, -605.1766, 28.49113), new Vector3(0.1124261, -0.245286, 33.12299), 89, 1, "BUS-30", "BUS");
        Main_VehicleManage.CreateVehicle(8, VehicleHash.Bus, new Vector3(461.7661, -611.5539, 28.48455), new Vector3(0.2906722, -0.593569, 34.92804), 89, 1, "BUS-30", "BUS");
        Main_VehicleManage.CreateVehicle(8, VehicleHash.Bus, new Vector3(460.6365, -618.6379, 28.49526), new Vector3(0.02470248, 0.01541019, 35.08606), 89, 1, "BUS-30", "BUS");
        Main_VehicleManage.CreateVehicle(8, VehicleHash.Bus, new Vector3(460.2524, -625.4366, 28.4907), new Vector3( 0.1228679, -0.2426173, 36.0867), 89, 1, "BUS-30", "BUS");
        Main_VehicleManage.CreateVehicle(8, VehicleHash.Bus, new Vector3(459.1967, -632.6961, 28.4958), new Vector3(0.03696952, 0.001246404, 35.07324), 89, 1, "BUS-30", "BUS");
        Main_VehicleManage.CreateVehicle(8, VehicleHash.Bus, new Vector3(458.6866, -639.7792, 28.48148), new Vector3(-0.6489474, 0.6727778, 35.1442), 89, 1, "BUS-30", "BUS");
        Main_VehicleManage.CreateVehicle(8, VehicleHash.Bus, new Vector3(458.0196, -646.998, 28.26405), new Vector3(-2.761997, -0.5089465, 34.31564), 89, 1, "BUS-30", "BUS");
        Main_VehicleManage.CreateVehicle(8, VehicleHash.Bus, new Vector3(457.1438, -654.2097, 27.86935), new Vector3(-3.038698, -2.348598, 34.52155), 89, 1, "BUS-30", "BUS");
    ...
    public static void onPlayerDissconnectedHandler(Client player, DisconnectionType type, string reason)
    {
        try
        {
            ...
            if (Main_Job.GetPlayerJob(player) == 8 &&
    ...
    private static void busCheckpointEnterWay(ColShape shape, Client player)
    {
        try
        {
            ...
            if (Main_Job.GetPlayerJob(player) != 8|| !player.GetData("ON_WORK") || player.GetData("WORKWAY") != shape.GetData("WORKWAY")) return;
            ...
            if (!BusWays[way][check].IsStop)
            {
                ...
                var payment = Convert.ToInt32(BuswaysPayments[way] * Main_Job.Payday_Job_Multipler) * 12;
            ...
            else
            {
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Halt. Nach 15 Sekunden können Sie die Route fortsetzen.", 5000);
               // player.SetData("BUS_TIMER", Timers.StartOnce(10000, () => timer_busStop(player, way, check)));
                player.SetData("BUS_TIMER", TimerEx.SetTimer(() =>  timer_busStop(player, way, check), 15000, 1));
     
                foreach (var p in Main.GetPlayersInRadiusOfPosition(player.Position, 30))
                    NAPI.Notification.SendNotificationToPlayer(p, "Nach 15 Sekunden verlässt der Bus die nächste Route " + BusWaysNames[way] + ".");
            }
        }
        catch (Exception ex) { Log.Write("busCheckpointEnterWay: " + ex.Message, nLog.Type.Error); }
    }

    public static void timer_busStop(Client player, int way, int check)
    {
        API.Shared.ConsoleOutput("111111");
        NAPI.Task.Run(() =>
        {
            try
            {
                API.Shared.ConsoleOutput("222222");
                NAPI.Data.SetEntityData(player, "BUS_ONSTOP", false);
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Você pode ir além", 5000);
                var payment = Convert.ToInt32(BuswaysPayments[way] * Main_Job.Payday_Job_Multipler) * 12;
                //NAPI.Data.SetEntityData(player, "PAYMENT", NAPI.Data.GetEntityData(player, "PAYMENT") + payment);
                Main.GivePlayerMoney(player, payment);
                if (check + 1 != BusWays[way].Count) check++;
            ...
            player.ResetData("BUS_TIMER");*/
                ...
                foreach (var p in Main.GetPlayersInRadiusOfPosition(player.Position, 30))
                    p.SendChatMessage("!{#3ADF00}O ônibus parte em uma rota " + BusWaysNames[way]);
            }
            catch (Exception e)
            {
    ...
    public void onPlayerExitVehicleHandler(Client player, Vehicle vehicle)
    {
        try
        {
            if (NAPI.Data.GetEntityData(vehicle, "TYPE") == "BUS" &&
                Main_Job.GetPlayerJob(player) == 8 &&
    ...
    public void onPlayerEnterVehicleHandler(Client player, Vehicle vehicle, sbyte seatid)
    {
        try
        {
            ...
            if (player.VehicleSeat == -1)
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Você não tem uma licença da categoria C", 5000);
                ...
                if (Main_Job.GetPlayerJob(player) == 8)
                {
                    if (vehicle.GetData("DRIVER") == null)
                    {
                        ...
                        {
                            if (Main.GetPlayerMoney(player) >= BusRentCost)
                            {

                                InteractMenu.ShowModal(player, "BUS_RENT", "LS Bus Center - Vermietung", "Möchten Sie diesen Bus für " + BusRentCost + " mieten?", "Ja", "Nein");

                            }
                            else
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Está faltando " + (BusRentCost - Main.GetPlayerMoney(player)) + "$ para alugar o ônibus", 5000);
                                player.WarpOutOfVehicle();
                            }
                        }
                        else
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Sie haben bereits einen gemieteten Bus", 5000);
                    }
                    else
                    {
                        if (NAPI.Data.GetEntityData(player, "WORK") != vehicle)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Der Bus hat bereits einen Fahrer", 5000);
                            player.WarpOutOfVehicle();
                        }
                        else
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Sie arbeiten nicht als Busfahrer. Sie können sich dem Job im LS Job Center anschließen", 5000);
                    player.WarpOutOfVehicle();
                }
            }
            else
            {
                if (NAPI.Data.GetEntityData(vehicle, "ON_WORK"))
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Sie haben den Fahrpreis von {price}$ bezahlt", 5000);
                        Main.GivePlayerMoney(player, -price);

                    }
                    else
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Sie haben nicht genug Geld, um den Fahrpreis zu bezahlen", 5000);
                        player.WarpOutOfVehicle();
                    }
                }
                else
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Im Moment ist kein Fahrer im Bus", 5000);
                    player.WarpOutOfVehicle();
                }
            }
        }
    ...
    public static void acceptBusRent(Client player)
    {
        if (NAPI.Player.IsPlayerInAnyVehicle(player) && player.VehicleSeat == -1 && player.Vehicle.GetData("TYPE") == "BUS")
        {
            ...
            {
                if (!player.GetData("status")) continue;
                if (Main_Job.GetPlayerJob(p) !=8 || !p.GetData("ON_WORK")) continue;
            ...
            Main.GivePlayerMoney(player, -BusRentCost);
            ...
            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Du hast einen Bus gemietet. Sie wurden der Route {BusWaysNames[way]} zugewiesen.", 5000);
        }
        else
        {
            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Sie müssen im Bus sein", 5000);
        }
    }
    ...
    {
        if(response == "BUS_RENT") acceptBusRent(player);
    }

    public static void ModalCancel(Client player, string response)
    {
        if (response == "BUS_RENT") if (player.IsInVehicle) player.WarpOutOfVehicle();
    }
