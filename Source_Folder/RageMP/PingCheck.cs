...
class PingCheck : Script
{
    ...
    {
        NAPI.Util.ConsoleOutput("[Loading]: PingCheck gestartet...");
    ...
    public void CheckPing()
    {
        ...
        foreach(Client c in player)
        {
            if(c.Ping > 55)
            {
                ...
                {
                    c.Kick($"Dein Ping liegt bei (~r~{c.Ping}~w~) und ist daher zu hoch!");
                    Warned.Remove(c);
                }
                else
                {
                    Main.SendErrorMessage(c, $"Dein Ping liegt bei (~r~{c.Ping}~w~) und ist daher zu hoch!");
                    Warned.Add(c);
                }
            }
            else
