...
mp.events.add("LoadXMR", (stations) => {
	if(!mp.browsers.exists(xmrCEF)) {
		xmrCEF = mp.browsers.new("package://factionlife/clientcef/xmr/cef/xmr.html");
	}
	...
	xmrInterval = setInterval(function() {
		let vid = null;
		if(mp.players.local.vehicle) vid = mp.players.local.vehicle.handle;
		let currMaxVol = 0.5;
...
mp.events.add("SetStation", (stations) => {
	...
	stations = JSON.parse(stations);
	if(!stations) {
		mp.gui.chat.push("Beim Abrufen der Stationen ist ein Fehler aufgetreten.");
	} else {
		let categories = [];
		for(let i = 0; i < stations.length; i++) {
		...
		let ui = new Menu("XMR-Station einstellen", "Wählen Sie ein Genre", new Point(50, screenRes.y * 0.3));
